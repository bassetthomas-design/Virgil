...
public sealed class ConfigService : IConfigService
{
    ...
    public ConfigService(){ _cfg = LoadMerged(); }
    ...
    {
        try{ if (File.Exists(path)) return JsonSerializer.Deserialize<VirgilConfig>(File.ReadAllText(path), new JsonSerializerOptions{PropertyNameCaseInsensitive=true}); } catch {}
        return null;
    }
    ...
    private static string MachinePath => Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "Virgil", "config.json");
    private static string UserPath    => Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),       "Virgil", "user.json");
}
